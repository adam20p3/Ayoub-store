...
export const buildProductWaMessage = (t, storeName, { name, category, price, imageUrl }) => {
  // WhatsApp uses the FIRST URL it detects in a message to generate the
  // rich link preview. By placing the product image URL on line 1 (and on
  // a line by itself) we make sure the chat shows the product photo as the
  ...
  lines.push(`*${storeName} — ${t.wa.headerSuffix}*`);
  lines.push('──────────────────────────');
  lines.push(`*${t.wa.product}:* ${name}`);
  lines.push(`*${t.wa.category}:* ${category || '—'}`);
  lines.push(`*${t.wa.price}:* ${price}`);
  lines.push('');
